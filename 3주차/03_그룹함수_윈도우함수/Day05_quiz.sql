...
LIMIT 10
;
--RANK() 에 왜 () 인지 ? SQLite 공식문서 가서 매개변수가 있는지 확인필요
...
SELECT
    ...
    , country
    , creditLimit
    , ROUND(AVG(creditLimit) OVER(PARTITION BY country), 2) AS avgCreditByCountry
FROM customers
...
SELECT
    ...
    , creditLimit
    , ROUND(AVG(creditLimit) OVER(PARTITION BY country), 2) AS avgCreditByCountry
    , creditLimit - ROUND(AVG(creditLimit) OVER(PARTITION BY country), 2) AS 증감액
FROM customers
ORDER BY country, creditLimit DESC
;
...
SELECT
    customerNumber
    , customerName
    -- 확장해서 생각 -> 궁금증. 시분초?? ==> 공식문서에 답이 존재 -> AI한테 샘플코드 요청해서 공부
    -- 깃허브 블로그에 정리하기.
    , strftime('%Y-%m', paymentDate) AS payYearMonth
    , AVG() OVER(PARTITION BY payYearMonth) avgMonthAmount
    , RANK() OVER(PARTITION BY avgMonthAmount) AS monthAvgRank
FROM customers
...
SELECT
    ...
    , paymentDate
    , amount
    , SUM(amount) OVER(ORDER BY paymentDate ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) runningTotal
-- ORDER BY 뒤에 ROWS.. 생략하면 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW 가 됨.
-- 따라서 중복값이 있을 땐 ROWS와 RANGE는 차이가 나기때문에 아무때나 생략할 수 없다.
FROM payments
...
SELECT
    ...
    , paymentDate
    , amount
    , LAG(amount, 1) OVER(PARTITION BY customerNumber ORDER BY paymentDate) AS prevAmount
FROM payments
...
SELECT
    DISTINCT country
    , c.customerName 
    , p.paymentDate AS firstPaymentDate
FROM (

    SELECT
        country
        , FIRST_VALUE(customerName) OVER(
            PARTITION BY country ORDER BY paymentDate DESC) AS firstCustomer
        , p.paymentDate 
    FROM customers c
    JOIN payments p ON c.customerNumber = p.customerNumber 
