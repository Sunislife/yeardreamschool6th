...
SELECT 
    ...
    , ROW_NUMBER() OVER(ORDER BY SUM(i.Total) DESC) AS ROW_NUMBER
FROM customers c
JOIN invoices i ON c.CustomerId = i.CustomerId
GROUP BY c.CustomerId, FirstName, LastName, Country
...
-- ROWS BETWEEN UNBOUDED PREECEDING 이 문법을 기억하자
...
SELECT
    ...
    , InvoiceDate
    , Total
    , LAG(Total, 2) OVER (ORDER BY InvoiceDate) AS PrevInvoiceTotal
...
SELECT
    ...
    , SUM(Total) / SUM(Total) OVER()  AS RevenueRatio
FROM invoices
...
SELECT 
    ...
    , ROUND(PERCENT_RANK() OVER (ORDER BY SUM(Total) DESC), 4) AS PctRANK
    , ROUND(CUME_DIST() OVER (ORDER BY SUM(Total) DESC), 4) AS CumeDist
FROM customers c
JOIN invoices i ON c.CustomerId = i.CustomerId
