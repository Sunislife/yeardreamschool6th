...
GROUP BY employeeNumber, rep_name
ORDER BY customer_count DESC
;
SELECT * FROM orders;
/* ----------------------------------------------------------------
...
SELECT
   c.customerNumber
   , c.customerName
   , COUNT(DISTINCT o.orderNumber) purchase_count
FROM customers c
JOIN orders o        ON c.customerNumber = o.customerNumber
...
/* ----------------------------------------------------------------
   Q17. [LAG] 월별 매출 및 전월 대비 증감 - 시계열 관련 함
...
/* ----------------------------------------------------------------
   Q18. [SUM OVER] productLine별 매출 비중 (Running ratio) 누적값
...
/* ----------------------------------------------------------------
   ...
   -- JOIN + 서브쿼리 + 윈도우 함수 + CTE -- WITH절사용하여 인라인뷰처럼.Common Table Explression
   --
   -- 요구사항:
