...
SELECT
   employeeNumber
   , firstName ||' '|| lastName AS rep_name
   , COUNT(*) AS customer_count
...
SELECT
   ...
   , ROUND(SUM(quantityOrdered * priceEach), 2) AS monthly_revenue
   , LAG(ROUND(SUM(quantityOrdered * priceEach),2)) OVER(
         ORDER BY strftime('%Y-%m', orderDate)
   ) AS prev_month_revenue
   , ROUND(
         SUM(quantityOrdered * priceEach) -
         LAG(SUM(quantityOrdered * priceEach)) OVER(
            ORDER BY strftime('%Y-%m', orderDate)
             ), 2
   )    AS mom_diff
FROM orders o
...
FROM (
    SELECT
        p.productLine,
        SUM(od.quantityOrdered * od.priceEach) AS line_revenue
    FROM products p
    JOIN orderdetails od
        ON p.productCode = od.productCode
    GROUP BY p.productLine
...
dashboard AS (
    SELECT
        ...
        AVG(line_revenue) OVER (
            PARTITION BY country
        ) AS country_avg_line_rev
...
SELECT
    ...
    CASE
        WHEN line_revenue > country_avg_line_rev
        THEN 'Y'
        ELSE 'N'
