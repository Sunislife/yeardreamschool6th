...
SELECT Country, COUNT(CustomerId) AS cnt_cus
FROM customers
GROUP BY Country
ORDER BY 2 DESC
;
...
-- 필드명이 같으면 반드시 Alias를 써주자.
SELECT tracks.Name AS T_name, genres.Name AS G_name
FROM tracks
JOIN genres
...
SELECT DISTINCT customers.FirstName AS CUS_Name
